...
static void bfin_dma_io_write(void *opaque, target_phys_addr_t addr,
...
static uint64_t bfin_dma_io_read(void *opaque, target_phys_addr_t addr, unsigned size)
{
    BfinDMAState *s = opaque;
...
static int bfin_dma_init(SysBusDevice *dev)
{
    BfinDMAState *s = FROM_SYSBUS(typeof(*s), dev);

    memory_region_init_io(&s->iomem, &bfin_dma_io_ops, s, "dma", mmr_size());
