...
static void bfin_evt_io_write(void *opaque, target_phys_addr_t addr,
...
static uint64_t bfin_evt_io_read(void *opaque, target_phys_addr_t addr, unsigned size)
{
    BfinEVTState *s = opaque;
...
static int bfin_evt_init(SysBusDevice *dev)
{
    BfinEVTState *s = FROM_SYSBUS(typeof(*s), dev);

    memory_region_init_io(&s->iomem, &bfin_evt_io_ops, s, "evt", mmr_size());
