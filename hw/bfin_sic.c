...
static void bfin_sic_io_write(void *opaque, target_phys_addr_t addr,
...
static uint64_t bfin_sic_io_read(void *opaque, target_phys_addr_t addr, unsigned size)
{
    BfinSICState *s = opaque;
...
static int bfin_sic_init(SysBusDevice *dev)
{
    BfinSICState *s = FROM_SYSBUS(typeof(*s), dev);

    memory_region_init_io(&s->iomem, &bfin_sic_io_ops, s, "sic", mmr_size());
