...
static void bfin_mmu_io_write(void *opaque, target_phys_addr_t addr,
...
static uint64_t bfin_mmu_io_read(void *opaque, target_phys_addr_t addr, unsigned size)
{
    BfinMMUState *s = opaque;
...
static int bfin_mmu_init(SysBusDevice *dev)
{
    BfinMMUState *s = FROM_SYSBUS(typeof(*s), dev);

    memory_region_init_io(&s->iomem, &bfin_mmu_io_ops, s, "mmu", mmr_size());
