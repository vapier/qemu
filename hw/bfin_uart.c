...
static void bfin_uart_io_write(void *opaque, target_phys_addr_t addr,
...
static uint64_t bfin_uart_io_read(void *opaque, target_phys_addr_t addr, unsigned size)
{
    BfinUARTState *s = opaque;
...
static int bfin_uart_init(SysBusDevice *dev)
{
    BfinUARTState *s = FROM_SYSBUS(typeof(*s), dev);

    memory_region_init_io(&s->iomem, &bfin_uart_io_ops, s, "uart", mmr_size());
