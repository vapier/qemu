...
CPUArchState *cpu_init(const char *cpu_model)
{
    ...
    cpu = BFIN_CPU(object_new(TYPE_BLACKFIN_CPU));
    env = &cpu->env;
...
#else
    TCGv pc = tcg_const_tl(dc->pc);
    gen_helper_require_supervisor(cpu_env, pc);
#endif
}
...
static void check_breakpoint(CPUState *cs, CPUArchState *env, DisasContext *dc)
{
    CPUBreakpoint *bp;
...
static void
gen_intermediate_code_internal(BlackfinCPU *cpu, TranslationBlock *tb,
                               int search_pc)
{
    CPUState *cs = CPU(cpu);
...
{
    gen_intermediate_code_internal(bfin_env_get_cpu(env), tb, 0);
}

void gen_intermediate_code_pc(CPUArchState *env, struct TranslationBlock *tb)
{
    gen_intermediate_code_internal(bfin_env_get_cpu(env), tb, 1);
}
